...
from portafolioapp32.forms import LoginForm
...
from django.contrib.contenttypes.models import ContentType
from portafolioapp32.autenticador import authenticatez
...
def vista_login(request):
	...
	if request.method == 'GET':
		context = LoginForm()
		return render (request, 'templates/login.html', {'form': context})
...
#ahora se captura los datos desde el frontend FUNCIONA OK
		usuario_frontend = request.POST.get('nickname', '')
		password_frontend = request.POST.get('password', '')	


# AHORA SE HACE EL FILTRADO

		x = q.filter(nickname__iexact= usuario_frontend)
...
# con fila accedo a la contrasena del objeto x creado con get_object_...
		try:
			if x and password_frontend == fila.password:
				user = authenticatez(nickname = x, password = fila.password)
	...
	# AHORA HAY QUE CREAR EN LA CACHE UNA VARIABLE DONDE SE ALMACENE fila.nombre_usuario_sys
				variable_cache = cache.set('uno', fila.nickname, None)


				#BLOQUE DE GRUPOS A CREAR
				g_contractors = Group.objects.get_or_create(name = 1)	
				g_developers = Group.objects.get_or_create(name = 2)
			




				sw = ContentType.objects.get_for_model(Usuario)
				...
				#	request.session['correo_usuario_actual'] = correo_usuario_actual
					...
					return redirect('/dashcontractor/')
				...
				#	request.session['correo_usuario_actual'] = correo_usuario_actual
					...
					return render(request, 'templates/show_dashboard_developer.html', data)
...
# y ademas quien_es (PARA LO DE BIENVENIDO(A), quien_es)
				...
				return redirect(LOGIN_REDIRECT_URL='dashcontractor/')
